...
class BeliefResolver:
    ...
    async def resolve(self, session_id: str, contradictions: List[Tuple[Belief, Belief, float, str]]) -> None:
        ...
        for old_b, new_b, score, reason in contradictions:
            if self.strategy == "overwrite":
                ...
                await self.store.remove_belief(session_id, old_b.subject, old_b.predicate)
                ...
                raise ValueError(f"Contradiction detected: {old_b.value} vs {new_b.value} - {reason}")
