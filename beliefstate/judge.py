import json
import logging
from typing import Any, Tuple, Protocol, runtime_checkable, Optional
...
class LLMJudge(ContradictionJudge):
    ...
    async def check(self, old: Belief, new: Belief) -> Tuple[bool, float, str]:
        ...
        try:
            ...
            if relationship == "contradiction" and score >= self.config.contradiction_threshold:
...
class LocalNLIJudge(ContradictionJudge):
    ...
    def __init__(self, model_name: str = "cross-encoder/nli-deberta-v3-xsmall", threshold: float = 0.7):
    ...
    async def check(self, old: Belief, new: Belief) -> Tuple[bool, float, str]:
        ...
        try:
            ...
            res = await loop.run_in_executor(
                None,
                lambda: self._pipeline({"text": premise, "text_pair": hypothesis})
            )
            ...
            if is_contra and score >= self.threshold:
                return True, score, f"Local NLI classification: {label} (score: {score:.2f})"
            return False, score, f"Local NLI classification: {label} (score: {score:.2f})"
        except Exception as e:
            logger.error(f"Local NLI judge error checking contradiction: {e}", exc_info=True)
            return False, 0.0, str(e)
