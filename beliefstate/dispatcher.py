...
def execute_tracking_task(call_dict: Dict[str, Any], response_dict: Dict[str, Any], session_id: str, turn: int) -> None:
    ...
    try:
        ...
        loop.run_until_complete(tracker.track_async(call_dict, response_dict, session_id, turn))
    finally:
        loop.close()
...
class TaskDispatcher(Protocol):
    ...
    def dispatch(self, tracker: Any, call: LLMCall, response: LLMResponse, session_id: str, turn: int) -> None:
...
class AsyncioDispatcher:
    ...
    def dispatch(self, tracker: Any, call: LLMCall, response: LLMResponse, session_id: str, turn: int) -> None:
        asyncio.create_task(
            tracker.track_async(call.model_dump(), response.model_dump(), session_id, turn)
        )
...
class SyncDispatcher:
    ...
    def dispatch(self, tracker: Any, call: LLMCall, response: LLMResponse, session_id: str, turn: int) -> None:
        ...
        asyncio.run(tracker.track_async(call.model_dump(), response.model_dump(), session_id, turn))


class CeleryDispatcher:
    """Dispatcher that serializes payloads and enqueues them via Celery."""
    def __init__(self, celery_app: Any = None, task_name: str = "beliefstate.dispatcher.execute_tracking_task", **kwargs: Any):
    ...
    def dispatch(self, tracker: Any, call: LLMCall, response: LLMResponse, session_id: str, turn: int) -> None:
        if not self.celery_app:
            raise RuntimeError("CeleryDispatcher requires a configured celery_app instance.")
...
class RQDispatcher:
    ...
    def __init__(self, queue: Any = None, queue_name: str = "default", connection: Any = None, **kwargs: Any):
        self.kwargs = kwargs
