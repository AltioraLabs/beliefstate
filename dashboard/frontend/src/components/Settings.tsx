...
export function Settings() {
  ...
  const handleReExtract = async () => {
    ...
    try {
      const res = await fetch(`/api/sessions/default/re-extract`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ custom_prompt: customPrompt, message: 'Re-extract with custom prompt' }),
      });
  ...
  const sections = [
    {
      ...
      fields: [
        ...
        { key: 'belief_sort_strategy', label: 'Sort Strategy', type: 'select',
          options: [
            {value:'confidence_recency', label:'Confidence + Recency'},
    ...
    {
      title: 'Detection Thresholds', icon: 'sliders',
      fields: [
    ...
    {
      title: 'Resolution', icon: 'git-merge',
      fields: [
        { key: 'resolution_strategy', label: 'Strategy', type: 'select',
          options: [
            {value:'overwrite', label:'Overwrite (replace old)'},
            {value:'keep_old', label:'Keep Old (ignore new)'},
            {value:'raise', label:'Raise Error'},
          ]
        },
  ...
  return (
    <div className="page settings-page">
      <div className="page-header">
        ...
        <p className="page-subtitle">Configure tracker behavior</p>
                ...
                return (
                  <div key={f.key} className="settings-field">
                    ...
                    {f.type === 'checkbox' ? (
                      ...
                      <select value={val || ''} onChange={e => updateField(f.key, e.target.value)} className="filter-select">
                        {f.options?.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                      </select>
                    ) : f.type === 'range' ? (
                      <div className="range-field">
                        <input type="range" min={f.min || 0} max={f.max || 1} step={0.01} value={val || 0} onChange={e => updateField(f.key, Number(e.target.value))} className="range-input" />
              ...
              {Object.entries(providerInfo).map(([name, info]: [string, any]) => (
                <div key={name} className="provider-card">
                  ...
                  <div className="provider-detail">
                    <span>Provider:</span><span className="font-mono">{info.name}</span>
                  </div>
                  {info.model && (
                    <div className="provider-detail">
                      <span>Model:</span><span className="font-mono">{info.model}</span>
                    </div>
                  )}
                </div>
              ))}
      ...
      <div className="glass-card">
        ...
        <div className="card-body">
          <textarea
            value={customPrompt || 'Enter custom prompt to test re-extraction...'}
            onChange={e => setCustomPrompt(e.target.value)}
            rows={10}
            className="sim-input font-mono text-sm"
          />
          <div className="flex gap-2 mt-3">
            <button className="btn btn-primary" onClick={handleReExtract}>Test Re-extract</button>
          </div>
          {reExtractResult && (
            <div className="mt-4">
              <span className="badge badge-assertion">Extracted {reExtractResult.count} beliefs</span>
              <pre className="sim-code mt-2">{JSON.stringify(reExtractResult.extracted, null, 2).slice(0, 2000)}</pre>
            </div>
          )}
        </div>
      </div>
