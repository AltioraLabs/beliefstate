...
export function Simulator({ sessionId }: Props) {
  ...
  const RenderBelief = ({ b }: { b: any }) => (
    <div className={`sim-belief ${b.is_hypothetical ? 'hypo' : ''}`}>
      <div className="sim-belief-head">
        <span className="sim-belief-title">{b.subject} {b.predicate}</span>
        <div className="sim-belief-tags">
      ...
      <div className="sim-belief-value">{b.value}</div>
      <div className="sim-belief-foot">
        <span className="confidence-dot" style={{ background: b.confidence >= 0.85 ? '#22c55e' : b.confidence >= 0.6 ? '#f59e0b' : '#ef4444' }} />
        <span className="sim-belief-conf">{(b.confidence * 100).toFixed(0)}%</span>
        {b.source_quote && <span className="sim-belief-quote">"{b.source_quote}"</span>}
      </div>
    </div>
  );
        ...
        <div className="card">
          <div className="sim-tabs">
            <button className={`sim-tab ${activeView === 'prompt' ? 'active' : ''}`} onClick={() => setActiveView('prompt')}>
              ...
              Injected Prompt
              {result.would_inject && <span className="badge badge-assertion">{(result.token_estimate || 0)} tokens</span>}
            </button>
            <button className={`sim-tab ${activeView === 'extracted' ? 'active' : ''}`} onClick={() => setActiveView('extracted')}>
              ...
              Extracted Beliefs
              <span className="badge badge-assertion">{result.extracted_beliefs?.length || 0}</span>
            </button>
          ...
          <div className="sim-output">
            {activeView === 'prompt' && (
              <div className="sim-prompt-block">
                <div className="sim-prompt-head">
                  <span className={`sim-prompt-status ${result.would_inject ? 'inject' : ''}`}>
                    {result.would_inject ? '✓ Would inject' : '✗ Would NOT inject'}
                  </span>
            ...
            {activeView === 'timing' && (
              <div className="sim-timing">
                {[
                  ...
                  <div key={i} className={`sim-timing-row ${item.isTotal ? 'total' : ''}`}>
                    <span className="sim-timing-label">{item.label}</span>
                    <div className="sim-timing-track">
                      <div className="sim-timing-fill" style={{width:`${item.pct}%`}} />
                    </div>
                    <span className="sim-timing-ms">{item.ms}ms</span>
                  </div>
                ))}
                <div className="sim-timing-extra">
