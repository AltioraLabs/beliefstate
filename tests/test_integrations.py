...
async def test_fastapi_middleware_context_propagation():
    ...
    scope = {
        "type": "http",
        "headers": [
            (b"x-session-id", b"session-fastapi-123")
        ]
    }
...
def test_flask_middleware_context_propagation():
    ...
    environ = {
        "HTTP_X_SESSION_ID": "session-flask-456"
    }
...
async def test_llamaindex_callback_handler():
    ...
    _, call_arg, response_arg, session_id_arg, turn_arg = mock_tracker.dispatcher.dispatch.call_args[0]
...
class MockOpenAIMessage:
    ...
    def model_dump(self) -> dict:
        return {
            "role": self.role,
            "run_id": self.run_id,
            "content": [{"type": "text", "text": {"value": self.content[0].text.value}}]
        }


def test_process_openai_assistant_message():
    thread_messages = [
        MockOpenAIMessage(role="assistant", content_val="This is the output of the run.", run_id="run_1"),
        MockOpenAIMessage(role="user", content_val="Who am I speaking with?"),
        MockOpenAIMessage(role="assistant", content_val="Older assistant reply", run_id="run_old"),
...
async def test_observe_run_polling_and_dispatch():
    ...
    mock_client.beta.threads.runs.retrieve = AsyncMock(side_effect=[mock_run_in_progress, mock_run_completed])
    
    mock_messages_cursor = MagicMock()
    mock_messages_cursor.data = [
        MockOpenAIMessage(role="assistant", content_val="Final assistant reply", run_id="run_abc"),
        MockOpenAIMessage(role="user", content_val="User prompt")
    ]
    mock_client.beta.threads.messages.list = AsyncMock(return_value=mock_messages_cursor)
    ...
    mock_client.beta.threads.messages.list.assert_called_once_with(thread_id="thread_123")
...
class MockLLMResult:
    ...
    def dict(self) -> dict:
        return {"generations": [[{"text": g.text} for g in gen_list] for gen_list in self.generations]}
...
async def test_langchain_callback_handler():
    ...
    # Simulating end event
    response = MockLLMResult(generations=[[MockGeneration(text="chicken crossed the road")]])
