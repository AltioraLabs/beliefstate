...
from beliefstate.call import LLMResponse, LLMCall
...
async def test_llm_judge_contradiction():
    ...
    b1 = Belief(subject="USER", predicate="lives in", value="Paris", confidence=1.0, turn=1, source="user")
    b2 = Belief(subject="USER", predicate="lives in", value="Tokyo", confidence=1.0, turn=2, source="user")
...
async def test_llm_judge_neutral():
    ...
    b1 = Belief(subject="USER", predicate="lives in", value="Paris", confidence=1.0, turn=1, source="user")
    b2 = Belief(subject="USER", predicate="likes", value="coffee", confidence=1.0, turn=2, source="user")
...
async def test_local_nli_judge_mock():
    ...
    try:
        ...
        mock_transformers.pipeline.assert_called_once_with("text-classification", model="cross-encoder/nli-deberta-v3-xsmall")
